...
export class AStar {
    ...
    private floydCrossAble(n1: PathNode, n2: PathNode): boolean {
        var ps: Vec2[] = this.bresenhamNodes(new Vec2(n1.x, n1.y), new Vec2(n2.x, n2.y));
    ...
    private bresenhamNodes(p1: Vec2, p2: Vec2): Vec2[] {
        ...
        if (steep) {
            ret.push(new Vec2(p1.y, p1.x));
        } else {
            ret.push(new Vec2(p1.x, p1.y));
        }
        while (nowX != p2.x) {
            var fy: number = Math.floor(nowY)
            var cy: number = Math.ceil(nowY);
            if (steep) {
                ret.push(new Vec2(fy, nowX));
            } else {
                ret.push(new Vec2(nowX, fy));
            }
            if (fy != cy) {
                if (steep) {
                    ret.push(new Vec2(cy, nowX));
                } else {
                    ret.push(new Vec2(nowX, cy));
                }
            }
            nowX += stepX;
            nowY += deltay;
        ...
        if (steep) {
            ret.push(new Vec2(p2.y, p2.x));
        } else {
            ret.push(new Vec2(p2.x, p2.y));
        }
        return ret;
    }
